...
export async function setupVite(app: Express, server: Server) {
  ...
  app.use("*", async (req, res, next) => {
    ...
    try {
      ...
      // Inject route-specific meta tags for /3 (Real Person Testimonial version)
      if (url === "/3" || url.startsWith("/3?") || url.startsWith("/3#")) {
        template = template.replace(
          "<title>仙佛護持・避邪淨化隨身噴霧｜啟動你的第一道結界</title>",
          "<title>仙佛護持・避邪淨化隨身噴霧｜真人實測・3000+人見證</title>"
        );
        template = template.replace(
          '<meta name="description" content="七大神聖精油配方・仙佛加持護持。一噴，像替自己升起一道防護罩。守住氣場・隔離外界干擾・隨時安心。" />',
          '<meta name="description" content="超過3,000人親身體驗，七大神聖植物精萃，仙佛護持加持。一噴淨化，啟動你的第一道結界。" />'
        );
        const ogTags3 = `
    <meta property="og:title" content="仙佛護持・避邪淨化隨身噴霧｜真人實測・3000+人見證" />
    ...
    <meta name="twitter:title" content="仙佛護持・避邪淨化隨身噴霧｜真人實測・3000+人見證" />
    ...
    <meta name="twitter:image" content="https://d2xsxph8kpxj0f.cloudfront.net/310519663413887714/VizcmhaMFeJmFoPCcusNnS/hero_woman_spray_18fd9677.png" />`;
        template = template.replace("</head>", `${ogTags3}\n  </head>`);
      }
...
export function serveStatic(app: Express) {
  ...
  app.use("*", (req, res) => {
    ...
    // Serve dedicated 2.html for /2 route (Money Magnet Mist) with correct OG meta tags
    if (url === "/2" || url.startsWith("/2?") || url.startsWith("/2#")) {
    ...
    // Serve dedicated 3.html for /3 route (Real Person Testimonial version) with correct OG meta tags
    if (url === "/3" || url.startsWith("/3?") || url.startsWith("/3#")) {
